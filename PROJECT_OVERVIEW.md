...
6. **发售日期识别**：自动检测游戏是否为预购状态，并在日志和通知中标记未发售游戏的发售日期
...
4. **发售日期识别**：自动提取游戏发售日期，判断是否为预购状态
5. **通知生成**：如果价格变动，生成通知内容并写入`result.md`文件，预购游戏会在标题中标注[预购]并显示发售日期
6. **日志更新**：将当前价格信息添加到日志文件中，预购游戏日志格式为 `[{时间}] {游戏名} [预购|发售:{日期}]: {价格}`

## 注意事项

- 脚本使用中国区价格（CNY），如需修改货币区域，可修改`get_game_price`函数中的`cc`参数
- 日志文件默认保留最近100行记录，可通过修改`MAX_LOG_LINES`变量调整
- 预购游戏（`coming_soon=True`）会在通知标题中标注 `[预购]`，并在日志中显示发售日期，方便区分已发售和即将发售的游戏
